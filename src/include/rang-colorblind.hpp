#ifndef RANGCOLORBLIND_DOT_HPP
#define RANGCOLORBLIND_DOT_HPP
...
enum class fg {
    black   = 30,
    red     = 32, //swapped red and green
    green   = 31, //swapped red and green
    yellow  = 33, 
    blue    = 34,
    magenta = 36, //swapped magenta and cyan
    cyan    = 35, //swapped magenta and cyan
    gray    = 37,
    reset   = 39
};
...
enum class bg {
    black   = 40,
    red     = 42, //swapped red and green
    green   = 41, //swapped red and green
    yellow  = 43,
    blue    = 44,
    magenta = 46, //swapped magenta and cyan
    cyan    = 45, //swapped magenta and cyan
    gray    = 47,
    reset   = 49
};
...
enum class fgB {
    black   = 90,
    red     = 92, //swapped red and green
    green   = 91, //swapped red and green
    yellow  = 93,
    blue    = 94,
    magenta = 96, //swapped magenta and cyan
    cyan    = 95, //swapped magenta and cyan
    gray    = 97
};

enum class bgB {
    black   = 100,
    red     = 102, //swapped red and green
    green   = 101, //swapped red and green
    yellow  = 103,
    blue    = 104,
    magenta = 106, //swapped magenta and cyan
    cyan    = 105, //swapped magenta and cyan
    gray    = 107
};
...
// Use rangcolorblind::setControlMode to set rang control mode
...
// Use rangcolorblind::setWinTermMode to explicitly set terminal API for Windows
// Calling rangcolorblind::setWinTermMode have no effect on other OS
...
namespace rang_implementation {
    ...
    using enableStd = typename std::enable_if<
      std::is_same<T, rangcolorblind::style>::value || std::is_same<T, rangcolorblind::fg>::value
        || std::is_same<T, rangcolorblind::bg>::value || std::is_same<T, rangcolorblind::fgB>::value
        || std::is_same<T, rangcolorblind::bgB>::value,
      std::ostream &>::type;
    ...
    inline void setWinSGR(rangcolorblind::bg col, SGR &state) noexcept
    {
        if (col != rangcolorblind::bg::reset) {
    ...
    inline void setWinSGR(rangcolorblind::fg col, SGR &state) noexcept
    {
        if (col != rangcolorblind::fg::reset) {
    ...
    inline void setWinSGR(rangcolorblind::bgB col, SGR &state) noexcept
    {
        state.bgColor = (BACKGROUND_INTENSITY >> 4)
          | ansi2attr(static_cast<BYTE>(col) - 100);
    }

    inline void setWinSGR(rangcolorblind::fgB col, SGR &state) noexcept
    {
        state.fgColor
          = FOREGROUND_INTENSITY | ansi2attr(static_cast<BYTE>(col) - 90);
    }

    inline void setWinSGR(rangcolorblind::style style, SGR &state) noexcept
    {
        switch (style) {
            case rangcolorblind::style::reset: state = defaultState(); break;
            case rangcolorblind::style::bold: state.bold = FOREGROUND_INTENSITY; break;
            case rangcolorblind::style::underline:
            case rangcolorblind::style::blink:
                state.underline = BACKGROUND_INTENSITY;
                break;
            case rangcolorblind::style::reversed: state.inverse = TRUE; break;
            case rangcolorblind::style::conceal: state.conceal = TRUE; break;
            default: break;
        }
    }
...
inline void setWinTermMode(const rangcolorblind::winTerm value) noexcept
{
