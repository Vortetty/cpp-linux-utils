...
void listDir(std::string path, bool p, bool s, bool b, bool v) {
    ...
    #ifdef _WIN32
        // from https://aerorock.co.nz/list-of-executable-file-extensions-windows/ minus .lnk
    ...
    if(termX < 31){
        while(listHeaderL + listHeaderR + 20 < termX) {
            listHeaderR += 1;

            if(listHeaderL + listHeaderR + 20 < termX) 
                listHeaderL += 1;
        }

        std::cout << std::string(listHeaderL, '-') << "---- " << rang::fgB::blue << "Directory "  << rang::fg::reset << "-----" << std::string(listHeaderR, '-') << "\n"
        << std::string(listHeaderL, '-') << "----- "<< rang::fgB::green << "Symlink " << rang::fg::reset << "------" << std::string(listHeaderR, '-') << "\n"
        << std::string(listHeaderL, '-') << "---- " << rang::fgB::magenta << "Executable " << rang::fg::reset << "----" << std::string(listHeaderR, '-') << "\n"
        << std::string(listHeaderL, '-') << "--- "<< rang::fgB::red << "Regular file " << rang::fg::reset << "---" << std::string(listHeaderR, '-') << "\n";
    }
    else if(termX > 20 && termX < 49){
    ...
    for (const auto & entry : fs::directory_iterator(path)){
        ...
        if(b){
            ...
            #else
                else if(_access((char*)entry.path().c_str(), X_OK) != -1){
                    sl size = parseSize(entry.file_size(), s);
                    std::cout << rangcolorblind::fgB::magenta << file << rangcolorblind::fg::reset << " - " << size.size << rangcolorblind::fgB::red << size.label << rangcolorblind::fg::reset;
                } 
            #endif
        ...
        if(p && !entry.is_directory()) {
            ...
            if(_access((char*)entry.path().c_str(), R_OK) != -1) std::cout << " Read";
            if(_access((char*)entry.path().c_str(), W_OK) != -1) std::cout << " Write";
            ...
            #else
                if(_access((char*)entry.path().c_str(), X_OK) != -1) std::cout << " Execute";
            #endif
        } else if (p) {
            std::cout << " -";
            if(_access((char*)entry.path().c_str(), R_OK) != -1) std::cout << " Read";
